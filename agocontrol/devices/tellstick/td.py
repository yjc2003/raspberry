...
# functionality.
#
# Check https://bitbucket.org/jorkar/pytelldus/commits/13edc6571ad17ce3d2a21fda948b2cee6e0e4d78#chg-tdtool.py
# for sensor handling
#
# *******************************************
# Change log
#
# 2014-01-31 Joakim Lindbom
...
def listSensors():
    ...
    while(tdlib.tdSensor(protocol, protocollength, model, modellength, byref(idvalue), byref(dataTypes)) == 0):
        ...
        if((dataTypes.value & TELLSTICK_TEMPERATURE) != 0):
            success = tdlib.tdSensorValue(protocol.value, model.value, idvalue.value, TELLSTICK_TEMPERATURE, value, valuelength, byref(timestampvalue))
            #print "Temperature: ", value.value, "C,", datetime.fromtimestamp(timestampvalue.value)
            s["temp"] = float(value.value)
        ...
        if((dataTypes.value & TELLSTICK_HUMIDITY) != 0):
            success = tdlib.tdSensorValue(protocol.value, model.value, idvalue.value, TELLSTICK_HUMIDITY, value, valuelength, byref(timestampvalue))
            #print "Humidity: ", value.value, "%,", datetime.fromtimestamp(timestampvalue.value)
            s["humidity"] = float(value.value)
